...
class NRRDPreprocessor(BasePreprocessor):
    ...
    Example:
        ...
        >>> 
        >>> # Simple loading (data only)
        >>> data = preprocessor.load("volume.nrrd")
    """
    ...
    def load(
        ...
        **kwargs,
        ...
        Args:
            ...
            index_order: Array index order ('C' for C-style row-major,
                'F' for Fortran-style column-major). Default: 'C'.
    ...
    def get_metadata(self, path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Convert header to regular dict and extract key info
            ...
            if space_directions is not None:
                ...
                for direction in space_directions:
                    if direction is not None:
                        ...
                        magnitude = np.sqrt(np.sum(np.array(direction) ** 2))
                        spacing.append(float(magnitude))
                return tuple(spacing) if spacing else None
            
            # Try 'spacings' field (simpler format)
