...
class BaseConnectomicsDataModule(pl.LightningDataModule, ABC):
    """
    ...
    BaseConnectomicsDataset subclass.
    ...
    Example:
        >>> class MyDataModule(BaseConnectomicsDataModule):
        ...     dataset_class = MyDataset
        >>> 
        >>> dm = MyDataModule(data_root="/path/to/data", batch_size=4)
        >>> dm.setup("fit")
        >>> train_loader = dm.train_dataloader()
    """
    
    dataset_class: Type[BaseConnectomicsDataset] = BaseConnectomicsDataset
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        if stage == "test" or stage is None:
            self.test_dataset = self.dataset_class(
                root_dir=self.data_root,
                split="test",
                cache_rate=0.0,  # Don't cache test data
                transform=self.get_val_transforms(),
                **extra_kwargs,
            )
