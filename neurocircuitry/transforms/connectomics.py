...
class ElasticDeformation(MapTransform, Randomizable):
    ...
    def __call__(self, data: Dict) -> Dict:
        ...
        # Get reference shape from first key
        ref_key = self.keys[0]
    ...
    def randomize(self, data: Optional[Dict] = None) -> None:
        ...
        if self._do_transform and data is not None:
            ...
            if ref_key in data:
                ...
                if depth > 2:
                    self._missing_idx = self.R.randint(1, depth - 1)
        ...
        for key in self.keys:
            if key in d and d[key].ndim >= 3:
                ...
                if self.fill_mode == "interpolate":
                    # Linear interpolation from neighboring slices
        ...
        for key in self.keys:
            if key in d:
                ...
                if self._defect_type == "line":
                    # Add horizontal or vertical line artifact
                    shape = arr.shape[-2:]
                    
                    if self.R.random() < 0.5:
                        # Horizontal line
                        y = self.R.randint(0, shape[0])
                        thickness = self.R.randint(1, 5)
                        intensity = self.R.uniform(0.5, 1.5)
                    ...
                    shift = self.R.uniform(-0.2, 0.2)
                    scale = self.R.uniform(0.8, 1.2)
                    arr = arr * scale + shift
